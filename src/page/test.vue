  ...
  methods: {
    ...
    getObjectURL(file) {
      ...
      if (window.createObjectURL!=undefined) { // basic
          urll = window.createObjectURL(file) ;
      } else if (window.URL!=undefined) { // mozilla(firefox)
          urll = window.URL.createObjectURL(file) ;
      } else if (window.webkitURL!=undefined) { // webkit or chrome
          urll = window.webkitURL.createObjectURL(file) ;
      }
      return urll ;
    },
    ...
    changeImg: function(e) {
      ...
      if (files.length > 0) {
        var dd = 0
        console.log('files.item(dd).size', files.item(dd).size)
    ...
    getData() { // 修改图片
      this.imgArr.splice(0, 1, 'http://sannuo-oss.oss-cn-shenzhen.aliyuncs.com/sino_wechat/education/files/202004/13/down158676730962719344-132075.jpg')
      this.allowAddImg = false
      this.url = 'http://sannuo-oss.oss-cn-shenzhen.aliyuncs.com/sino_wechat/education/files/202004/13/down158676730962719344-132075.jpg'
    },
    test () {
      var sqlx = "1";
      var dj ="10";
